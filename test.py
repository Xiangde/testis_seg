...
from dataset.dataset_test import Test_Datasets,to_one_hot_3d
...
def predict_one_img(model, img_dataset, args):
    ...
    if args.n_labels==2: 
        test_dice = OrderedDict({'Dice_testis': test_dice.avg[1]})
    if args.n_labels==3: 
        aaa = test_dice.avg[2]
        test_dice = OrderedDict({'Dice_testis': test_dice.avg[1]})
        test_dice.update({'Dice_tumor': aaa})   
...
if __name__ == '__main__':
    args = config.args
    save_path = os.path.join('xx', 'ResUNet-train') # your results path
